...
from telegram import (Update, InlineKeyboardButton, InlineKeyboardMarkup,
                      BotCommand, BotCommandScopeAllPrivateChats, BotCommandScopeAllGroupChats, ForceReply)
...
TZ_DATA = [
    ("Asia/Kolkata","India","+5:30","Asia"),("Asia/Dubai","UAE","+4","Asia"),
    ("Asia/Karachi","Pakistan","+5","Asia"),("Asia/Dhaka","Bangladesh","+6","Asia"),
    ("Asia/Bangkok","Thailand","+7","Asia"),("Asia/Singapore","Singapore","+8","Asia"),
    ("Asia/Shanghai","China","+8","Asia"),("Asia/Tokyo","Japan","+9","Asia"),
    ("Asia/Seoul","Korea","+9","Asia"),("Asia/Jakarta","Indonesia","+7","Asia"),
    ("Asia/Riyadh","Saudi Arabia","+3","Asia"),("Asia/Manila","Philippines","+8","Asia"),
    ("Europe/London","UK","0/+1","Europe"),("Europe/Berlin","Germany","+1/+2","Europe"),
    ("Europe/Paris","France","+1/+2","Europe"),("Europe/Moscow","Russia","+3","Europe"),
    ("Europe/Istanbul","Turkey","+3","Europe"),
    ("America/New_York","US East","-5/-4","Americas"),("America/Chicago","US Central","-6/-5","Americas"),
    ("America/Denver","US Mountain","-7/-6","Americas"),("America/Los_Angeles","US West","-8/-7","Americas"),
    ("America/Sao_Paulo","Brazil","-3","Americas"),("America/Mexico_City","Mexico","-6/-5","Americas"),
    ("Australia/Sydney","Australia","+10/+11","Oceania"),("Pacific/Auckland","New Zealand","+12/+13","Oceania"),
    ("Africa/Lagos","Nigeria","+1","Africa"),("Africa/Cairo","Egypt","+2","Africa"),
    ("Africa/Nairobi","Kenya","+3","Africa"),("Africa/Johannesburg","S. Africa","+2","Africa"),
]
...
ST_IC = {"active":"○","pending":"●","missed":"✗","snoozed":"◷","cancelled":"✕"}
ST_LB = {"active":"Active","pending":"Pending","missed":"Missed","snoozed":"Snoozed","cancelled":"Cancelled"}
...
def norm_time(val):
    ...
    if ":" in s:
        try:
            p = s.split(":"); h,m = int(p[0]), int(p[1].split()[0])
            u = s.upper()
...
def get_cfg(uid):
    ...
    try: rows = cfg_sheet.get_all_values()
        ...
        except: return {"digest_on":True,"digest_time":DEF_DIGEST_TIME,"max_retries":DEF_RETRIES,"retry_gap":DEF_RETRY_GAP,"timezone":DEF_TZ}
    for r in rows[1:]:
        if str(r[0]) == uid_s:
...
def update_username(uid, username):
    ...
    for i,r in enumerate(rows[1:],2):
        if str(r[0]) == uid_s:
            cur = r[6].strip().lower() if len(r)>6 else ""
            if cur != uname: cfg_sheet.update_cell(i,7,uname)
            return
    get_cfg(uid)
    try: rows = cfg_sheet.get_all_values()
    except: return
    for i,r in enumerate(rows[1:],2):
        if str(r[0]) == uid_s: cfg_sheet.update_cell(i,7,uname); return
...
def advance_rep(row, r):
    ...
    if rep == "daily": nd = d + timedelta(days=1)
        ...
        for offset in range(1,8):
            ...
            if candidate.strftime("%a").lower()[:3] in days: nd = candidate; break
        else: return False
    else: return False
...
# ============= GROUP DATA =================
def gen_tid(): return f"t{int(time_mod.time())}"

def grp_read(sh, fn):
    try: rows = sh.get_all_values()
    except: return []
    return [r for r in rows[1:] if fn(r)]
...
def set_gsub(gid, uid, name, username="", sub=True):
    ...
    for i,r in enumerate(rows[1:],2):
        if str(r[0])==gid_s and str(r[1])==uid_s:
            ...
            grp_sheet.update_cell(i,5,str(sub).lower()); return
...
def add_tmember(tid, uid, name, st="waiting"):
    ...
    task_sheet.append_row([str(tid),uid_s,name,st], value_input_option="RAW"); return True
...
def gstatus_text(tid, msg):
    ...
    di = "⏳"
    pfx = "⏰ " if any(s not in ("done","missed") for _,_,s in active) else ""
    parts = [f"{GT_IC.get(s,di)} {n}" for _,n,s in active]
    return f"{pfx}{msg}\n\n{' · '.join(parts)}"
...
def extract_tags(message):
    tags = []
    if not message.entities: return tags
    for entity in message.entities:
        if entity.type == "text_mention" and entity.user:
            if entity.user.first_name: tags.append(entity.user.first_name.lower().strip())
            if entity.user.username: tags.append(entity.user.username.lower().strip())
...
def schedule_minimize(ctx, msg, min_text, show_cb):
    ctx.job_queue.run_once(auto_minimize, AUTO_MIN_SEC, data={
        "c": msg.chat.id, "m": msg.message_id, "min_text": min_text, "show_cb": show_cb})
    ctx.bot_data[f"gmin_{msg.message_id}"] = {"min_text": min_text, "show_cb": show_cb}

# ============= UI ========================
HOME_TEXT = (f"{hdr(BOT_NAME)}\n"
...
def close_kb(show_cb): return InlineKeyboardMarkup([[InlineKeyboardButton("- Close", callback_data=f"pclose_{show_cb}")]])
def show_kb(show_cb): return InlineKeyboardMarkup([[InlineKeyboardButton("📋 Show", callback_data=show_cb)]])
...
def saved_kb(row, rep):
    r = str(rep)
    if r == "none" or not r:
        return InlineKeyboardMarkup([[InlineKeyboardButton("🔁 Repeat", callback_data=f"chrep_{row}"), InlineKeyboardButton("✎ Edit", callback_data=f"edit_{row}")],
                                     [InlineKeyboardButton("＋ New", callback_data="add")]])
    return InlineKeyboardMarkup([[InlineKeyboardButton("✎ Edit", callback_data=f"edit_{row}"), InlineKeyboardButton("＋ New", callback_data="add")]])
...
def custom_days_kb(selected, prefix):
    row1 = [InlineKeyboardButton(f"[{DAY_NAMES[i]}]" if DAY_KEYS[i] in selected else DAY_NAMES[i], callback_data=f"cday_{prefix}_{DAY_KEYS[i]}") for i in range(4)]
    row2 = [InlineKeyboardButton(f"[{DAY_NAMES[i]}]" if DAY_KEYS[i] in selected else DAY_NAMES[i], callback_data=f"cday_{prefix}_{DAY_KEYS[i]}") for i in range(4,7)]
    btns = [row1, row2,
        [InlineKeyboardButton("Mon-Fri", callback_data=f"cday_{prefix}_weekdays"), InlineKeyboardButton("All", callback_data=f"cday_{prefix}_all")],
        [InlineKeyboardButton("Clear", callback_data=f"cday_{prefix}_clear")]]
    if selected: btns.append([InlineKeyboardButton("✓ Save", callback_data=f"cdaysave_{prefix}")])
...
def cal_kb(year, month, back_cb="cancel", back_txt="✕ Cancel", tz=None):
    now = datetime.now(tz or safe_tz(DEF_TZ))
    kb = [[InlineKeyboardButton(f"{cal_mod.month_name[month]} {year}", callback_data="noop")]]
...
def _find_relative(text, tz=None):
    ...
    if m and int(m.group(1))>0:
        tgt = now + timedelta(minutes=int(m.group(1)))
        return tgt.strftime("%Y-%m-%d"), tgt.strftime("%H:%M"), m.start(), m.end()
    m = re.search(r'\b(?:in|after)\s+(\d+)\s*(?:hour|hr|h)s?\b', low)
    if m and int(m.group(1))>0:
        tgt = now + timedelta(hours=int(m.group(1)))
        return tgt.strftime("%Y-%m-%d"), tgt.strftime("%H:%M"), m.start(), m.end()
    m = re.search(r'\b(?:in|after)\s+(\d+)\s*days?\b', low)
    if m and int(m.group(1))>0:
        tgt = now + timedelta(days=int(m.group(1)))
        return tgt.strftime("%Y-%m-%d"), now.strftime("%H:%M"), m.start(), m.end()
    m = re.search(r'\b(?:in|after)\s+(\d+)\s*weeks?\b', low)
    if m and int(m.group(1))>0:
        tgt = now + timedelta(weeks=int(m.group(1)))
        return tgt.strftime("%Y-%m-%d"), now.strftime("%H:%M"), m.start(), m.end()
    return None

def _find_date(text, tz=None):
    now = datetime.now(tz or safe_tz(DEF_TZ)); low = text.lower()
...
def _find_repeat(text, tz=None):
    low = text.lower(); now = datetime.now(tz or safe_tz(DEF_TZ))
    days_full = ['monday','tuesday','wednesday','thursday','friday','saturday','sunday']
    days_abr = ['mon','tue','wed','thu','fri','sat','sun']
    for i,(full,abr) in enumerate(zip(days_full,days_abr)):
    ...
    for pat, rep in [(r'\b(?:every\s*day|daily|every\s*hour)\b','daily'),(r'\b(?:every\s*week|weekly)\b','weekly'),
...
def _clean(text, spans):
    ...
    for f in [r'^\s*remind\s+(?:me\s+)?to\s+', r'^\s*remind\s+to\s+', r'^\s*reminder\s+to\s+', r'^\s*reminder\s+',
...
def parse_nl_partial(text, tz=None):
    ...
    if rel:
        ds_r, ts_r, rs, re_ = rel
        rr = _find_repeat(text, tz)
        rep = rr[0] if rr else None
        spans = [(rs, re_)]
        if rr: spans.append((rr[1],rr[2]))
        msg = _clean(text, spans)
        if not msg: return None
        return {'message':msg,'date':ds_r,'time':ts_r,'repeat':rep}
    tr = _find_time(text)
    ...
    rep = rr[0] if rr else None
    if rr and len(rr)>3 and rr[3]: ds = rr[3]
    spans = []
    if tr: spans.append((tr[1],tr[2]))
    if dr: spans.append((dr[1],dr[2]))
    if rr: spans.append((rr[1],rr[2]))
    msg = _clean(text, spans)
    if not msg: return None
...
async def post_init(app):
    await app.bot.set_my_commands([
        ...
        BotCommand("month","Monthly schedule"), BotCommand("settings","Bot settings"),
        BotCommand("info","About this bot"),
    ], scope=BotCommandScopeAllPrivateChats())
    await app.bot.set_my_commands([
        BotCommand("start","Bot info & commands"), BotCommand("remind","Group reminder"),
        BotCommand("list","Active reminders"),
    ], scope=BotCommandScopeAllGroupChats())
    await app.bot.set_my_commands([])

GRP_START = (f"{hdr(BOT_NAME)}\n\n"
...
def build_grp_list_text(gid):
    ...
    items = [(i,r) for i,r in enumerate(rows[1:],2) if len(r)>7 and str(r[7]).strip()==str(gid) and str(r[5]).strip() in ("active","pending","snoozed")]
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        gid = str(update.effective_chat.id); user = update.effective_user
        ...
        schedule_minimize(ctx, sent, f"<b>{BOT_NAME}</b>", f"gshow_start_{sent.message_id}")
        ctx.bot_data[f"gmin_{sent.message_id}"]["full_text"] = GRP_START
        return
    await rm_home(ctx, ctx.user_data); ctx.user_data.clear()
    user = update.effective_user; get_cfg(user.id); update_username(user.id, get_username(user))
...
async def list_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        ...
        if not list_text:
            ...
            schedule_minimize(ctx, sent, "<b>Group Reminders</b> — No active", f"gshow_list_{gid}_{sent.message_id}")
            return
        ...
        ctx.bot_data[f"gmin_{sent.message_id}"]["gid"] = gid
        return
    await rm_home(ctx, ctx.user_data); ctx.user_data.clear()
    update_username(update.effective_user.id, get_username(update.effective_user))
...
async def info_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    info_text = (
        ...
        "<code>Meeting every monday at 10am</code>\n\n"
        "<i>Add daily, weekly, monthly to repeat.</i>\n\n"
        "<b>Features</b>\n"
        "• Smart natural language input\n"
        "• Calendar picker & flexible time\n"
        "• Snooze 15m-12h · Auto-retry\n"
        "• Custom days (Mon-Fri)\n"
        "• Daily digest · Weekly report\n"
        "• Monthly schedule (/month)\n"
        "• Group reminders (/remind)\n"
        f"• Timezone: {tz_short(cfg['timezone'])}\n\n"
        "<b>Commands</b>\n"
        "/add · /list · /month · /settings · /info\n"
        "Groups: /remind · /list · /start")
    sent = await update.message.reply_text(info_text, reply_markup=close_kb("pshow_info"), parse_mode="HTML")
    ctx.bot_data[f"pinfo_{sent.message_id}"] = {"text": info_text, "uid": update.effective_user.id}
    schedule_minimize(ctx, sent, f"<b>ℹ️ {BOT_NAME} Info</b>", "pshow_info")
...
async def month_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await rm_home(ctx, ctx.user_data); ctx.user_data.clear()
    update_username(update.effective_user.id, get_username(update.effective_user))
    uid = update.effective_user.id; utz = get_tz(uid); now = datetime.now(utz)
    ...
    schedule_minimize(ctx, sent, f"<b>📅 {now.strftime('%B %Y')}</b>", f"pshow_month_{now.year}_{now.month:02d}")
...
async def remind_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    uid = update.effective_user.id; utz = get_tz(uid); gid = str(update.effective_chat.id)
    user = update.effective_user; name = user.first_name or "User"; uname = get_username(user)
    ...
    if not text:
        ...
        sent = await update.message.reply_text(
            ...
            reply_markup=ForceReply(selective=True, input_field_placeholder="Type reminder..."), parse_mode="HTML")
        save_p(ud, sent); return
    result = parse_nl_partial(text, tz=utz)
    if result and result.get('message'):
        msg = result['message']; ud["message"] = msg
        ...
        ud["message"] = text; ud["step"] = "g_date"; now = datetime.now(utz)
...
async def show_settings(target, uid, ctx, new=False):
    ...
    if new:
        ...
        schedule_minimize(ctx, sent, "<b>⚙️ Settings</b>", "pshow_settings")
    else:
...
def expand_recur(reminders, start_date, end_date):
    expanded = []
    for r in reminders:
        msg,ds,ts = str(r[1]).strip(), norm_date(r[2]), norm_time(r[3])
        ...
        entry = lambda d, s=st: {"date":d,"msg":msg,"time":ts,"rep":rep,"status":s}
        if rep=="none" or not rep:
            if start_date<=rem_date<=end_date: expanded.append(entry(rem_date))
        elif rep=="daily":
            d = max(rem_date, start_date)
            while d<=end_date: expanded.append(entry(d, st if d==rem_date else "active")); d+=timedelta(days=1)
        elif rep=="weekly":
            d = rem_date
            while d<=end_date:
                if d>=start_date: expanded.append(entry(d, st if d==rem_date else "active"))
                d+=timedelta(days=7)
        elif rep=="monthly":
            d = rem_date
            for _ in range(12):
                if start_date<=d<=end_date: expanded.append(entry(d, st if d==rem_date else "active"))
                mo,yr = d.month+1,d.year
                if mo>12: mo,yr=1,yr+1
            ...
            d = max(rem_date, start_date)
            while d<=end_date:
                if d.strftime("%a").lower()[:3] in cdays: expanded.append(entry(d, st if d==rem_date else "active"))
                d+=timedelta(days=1)
    return expanded
...
def build_month_view(uid, year, month, utz):
    ...
    last_day = datetime(year,month,cal_mod.monthrange(year,month)[1]).date()
    ...
    d = first_day
    while d <= last_day:
        ws = d; we = min(d+timedelta(days=6-d.weekday()), last_day)
        wi = [x for x in expanded if ws<=x["date"]<=we]
        weeks.append({"start":ws,"end":we,"count":len(wi)})
        d = we+timedelta(days=1)
    # Merge to max 4
    while len(weeks)>4:
        min_i = min(range(len(weeks)-1), key=lambda i: weeks[i]["count"]+weeks[i+1]["count"])
        weeks[min_i] = {"start":weeks[min_i]["start"],"end":weeks[min_i+1]["end"],
                        "count":weeks[min_i]["count"]+weeks[min_i+1]["count"]}
        weeks.pop(min_i+1)
    total = len(expanded)
    done_c = sum(1 for x in expanded if x["status"]=="done")
    miss_c = sum(1 for x in expanded if x["status"]=="missed")
    up_c = total - done_c - miss_c
    mname = cal_mod.month_name[month]
    lines = [f"📅 <b>{mname} {year}</b>\n{DIV}\n"]
    for i,w in enumerate(weeks):
        ws_s, we_s = w["start"].strftime("%-d %b"), w["end"].strftime("%-d %b")
        cur = " ◂" if w["start"]<=now.date()<=w["end"] else ""
        lines.append(f"<b>W{i+1}</b>: {ws_s} – {we_s}{cur} · {w['count']}")
    if total:
        parts = []
        if done_c: parts.append(f"✅ {done_c}")
        if miss_c: parts.append(f"✗ {miss_c}")
        if up_c: parts.append(f"○ {up_c}")
        lines.append(f"\nTotal: {total} · {' · '.join(parts)}")
    # Buttons 2 per line
    btns = []
    for i in range(0, len(weeks), 2):
        row = [InlineKeyboardButton(f"W{i+1}", callback_data=f"mw_{year}_{month:02d}_{i}")]
        if i+1 < len(weeks): row.append(InlineKeyboardButton(f"W{i+2}", callback_data=f"mw_{year}_{month:02d}_{i+1}"))
        btns.append(row)
    pm,py = ((month-2)%12)+1, year-(1 if month==1 else 0)
    nm,ny = (month%12)+1, year+(1 if month==12 else 0)
    btns.append([InlineKeyboardButton(f"‹ {cal_mod.month_abbr[pm]}", callback_data=f"mn_{py}_{pm:02d}"),
                 InlineKeyboardButton(f"{cal_mod.month_abbr[nm]} ›", callback_data=f"mn_{ny}_{nm:02d}")])
...
def build_week_view(uid, year, month, week_idx, utz):
    ...
    while d<=last_day:
        ws = d; we = min(d+timedelta(days=6-d.weekday()),last_day)
        weeks.append((ws,we)); d = we+timedelta(days=1)
    while len(weeks)>4:
        min_i = min(range(len(weeks)-1), key=lambda i: 0)
        weeks[min_i] = (weeks[min_i][0], weeks[min_i+1][1])
        weeks.pop(min_i+1)
    if week_idx>=len(weeks):
        return f"{hdr('Week')}\nInvalid.", InlineKeyboardMarkup([[InlineKeyboardButton("« Back", callback_data=f"mn_{year}_{month:02d}")]])
    ws, we = weeks[week_idx]
    reminders = get_user_reminders(uid)
    expanded = expand_recur(reminders, ws, we)
    by_date = {}
    for item in expanded: by_date.setdefault(item["date"],[]).append(item)
    # Separate singles vs recurring for display
    recur_keys = {}
    for item in expanded:
        if item["rep"] and item["rep"]!="none":
            key = f"{item['msg']}|{item['time']}|{item['rep']}"
            recur_keys.setdefault(key,{"msg":item["msg"],"time":item["time"],"rep":item["rep"],"dates":[]})
            recur_keys[key]["dates"].append(item["date"])
    lines = [f"<b>W{week_idx+1}</b>: {ws.strftime('%-d %b')} – {we.strftime('%-d %b')}\n{DIV}\n"]
    # Show day-by-day for one-time reminders
    d = ws
    while d<=we:
        day_items = by_date.get(d,[])
        singles = [x for x in day_items if x["rep"]=="none" or not x["rep"]]
        if singles:
            dl = f"Today, {d.strftime('%-d %b, %a')}" if d==now.date() else d.strftime("%-d %b, %a")
            lines.append(f"<b>{dl}</b>")
            for item in sorted(singles, key=lambda x:x["time"]):
                ic = ST_IC.get(item["status"],"○")
                lines.append(f"  {ic} {fmt_time(item['time'])} · {item['msg']}")
            lines.append("")
        d += timedelta(days=1)
    # Show recurring at bottom with label
    if recur_keys:
        for grp in recur_keys.values():
            rep_label = fmt_rep(grp["rep"])
            lines.append(f"<i>{rep_label}</i>\n  ○ {fmt_time(grp['time'])} · {grp['msg']}")
        lines.append("")
    if not expanded: lines.append("No reminders this week.")
    mname = cal_mod.month_name[month]
    btns = []
    # Next/prev week navigation — 2 per line
    nav_row = []
    if week_idx > 0: nav_row.append(InlineKeyboardButton(f"‹ W{week_idx}", callback_data=f"mw_{year}_{month:02d}_{week_idx-1}"))
    if week_idx < len(weeks)-1: nav_row.append(InlineKeyboardButton(f"W{week_idx+2} ›", callback_data=f"mw_{year}_{month:02d}_{week_idx+1}"))
    if nav_row: btns.append(nav_row)
    btns.append([InlineKeyboardButton(f"« {mname} {year}", callback_data=f"mn_{year}_{month:02d}")])
    return "\n".join(lines), InlineKeyboardMarkup(btns)

# ============= SAVE FUNCTIONS =============
async def save_reminder(target, uid, ud, msg, date, time_s, edit_msg=False):
    rep = ud.get("repeat","none")
    sheet.append_row([uid,msg,date,time_s,rep,"active",0,"",""], value_input_option="RAW")
    try: row = len(sheet.get_all_values())
    except: row = 0
    ud.clear()
    txt = f"{hdr('Saved ✓')}\n{detail(msg,date,time_s,fmt_rep(rep))}"
...
async def finish_group_remind(target, ctx, uid, ud, rep, edit_msg=False):
    ...
    gid, name = ud.get("g_chat",""), ud.get("g_name","User")
    tags = ud.get("g_tags"); tid = gen_tid()
    sheet.append_row([uid,msg,ds,ts,rep,"active",0,gid,tid], value_input_option="RAW")
    subs = get_gsubs(gid)
    if tags:
        tagged_names = []
        for sub_uid, sub_name, sub_uname in subs:
            sub_name_l, sub_uname_l = sub_name.lower().strip(), sub_uname.lower().strip() if sub_uname else ""
            matched = any(tag==sub_uname_l and sub_uname_l for tag in tags) or any(tag==sub_name_l for tag in tags)
            if matched: add_tmember(tid,sub_uid,sub_name,"waiting"); tagged_names.append(sub_name)
            else: add_tmember(tid,sub_uid,sub_name,"skipped")
        sub_info = f"For: {', '.join(tagged_names)}" if tagged_names else "No matching subscribers"
    else:
        for sub_uid,sub_name,sub_uname in subs: add_tmember(tid,sub_uid,sub_name)
        sub_info = gsub_text(tid)
    txt = f"{hdr('Group Reminder')}\n{detail(msg,ds,ts,fmt_rep(rep))}\nBy {name}\n\n{sub_info}"
    show_rep = (rep=="none")
    kb = gjoin_kb(tid, show_rep); ud.clear()
    if edit_msg:
...
async def handle_nl_result(target, ctx, uid, ud, msg, ts, ds, utz, is_group=False):
    if ts:
        ...
        if is_past(ds, ts, utz):
            ...
            ud["date"] = ds
            if is_group: await finish_group_remind(target,ctx,uid,ud,ud.get("repeat","none"))
            else: await save_reminder(target,uid,ud,msg,ds,ts)
    elif ds:
        ud["date"] = ds
        ...
        if is_group:
            sent = await target.reply_text(
                f"{hdr(title)}\n{msg}\n{fmt_date(ds)}\n\nEnter time:\n<i>↩️ Reply to this</i>\n<i>e.g. 9pm, 9:30 PM</i>",
                reply_markup=ForceReply(selective=True,input_field_placeholder="e.g. 9pm"), parse_mode="HTML")
        else:
...
async def show_list(target, uid, ctx, new=False):
    ...
    if not items:
        t = f"{hdr('Reminders')}\nNo reminders found."
        kb = close_kb("pshow_list")
        if new:
            sent = await target.reply_text(t, reply_markup=kb, parse_mode="HTML")
            schedule_minimize(ctx, sent, "<b>📋 Reminders</b> (0)", "pshow_list")
    ...
    btns.append([InlineKeyboardButton("- Close", callback_data="pclose_pshow_list")])
    if new:
        sent = await target.reply_text("\n".join(lines), reply_markup=InlineKeyboardMarkup(btns), parse_mode="HTML")
        schedule_minimize(ctx, sent, f"<b>📋 Reminders</b> ({len(items)})", "pshow_list")
    else:
...
async def on_btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="noop": return
    if update.effective_chat.type=="private": update_username(uid, get_username(q.from_user))
    ...
    if data=="gclose":
        ...
        if stored:
            ...
            await safe_edit(q.message, f"<b>{BOT_NAME}</b>", gmin_kb(f"gshow_generic_{mid}"))
        return
    ...
    if data.startswith("pclose_"):
        ...
        if "list" in show_cb:
            try:
                uid_s = str(uid); rr = sheet.get_all_records()
                cnt = sum(1 for r in rr if str(r.get("user_id",""))==uid_s and str(r.get("status","")).strip() in ("active","pending","missed","snoozed") and not str(r.get("group_id","")).strip())
            except: cnt = "?"
            await safe_edit(q.message, f"<b>📋 Reminders</b> ({cnt})", show_kb(show_cb))
        elif "info" in show_cb:
            await safe_edit(q.message, f"<b>ℹ️ {BOT_NAME} Info</b>", show_kb(show_cb))
        elif "month" in show_cb:
            parts = show_cb.split("_"); yr,mo = parts[-2],parts[-1]
            mname = cal_mod.month_name[int(mo)]
            await safe_edit(q.message, f"<b>📅 {mname} {yr}</b>", show_kb(show_cb))
        elif "settings" in show_cb:
            await safe_edit(q.message, "<b>⚙️ Settings</b>", show_kb(show_cb))
        else:
            await safe_edit(q.message, f"<b>{BOT_NAME}</b>", show_kb(show_cb))
        return

    # Private show (expand)
    if data=="pshow_info": await info_cmd(update, ctx); return
    if data=="pshow_list": await show_list(q.message, uid, ctx); return
    if data=="pshow_settings": await show_settings(q.message, uid, ctx); return
    ...
    if data.startswith("gshow_list_"):
        parts = data[11:]; sep = parts.rfind("_")
        gid = parts[:sep] if sep>0 else str(q.message.chat.id)
        list_text, _ = build_grp_list_text(gid)
        if list_text: await safe_edit(q.message, list_text, gclose_kb())
        else: await safe_edit(q.message, f"{hdr('Group Reminders')}\nNo active reminders.", gclose_kb())
        return
    ...
    if data=="add":
        ...
        save_p(ud, sent); return
    if data=="list_refresh": ud.clear(); await show_list(q.message, uid, ctx); return

    # Settings close → minimize
    if data=="cfg_close":
        await safe_edit(q.message, "<b>⚙️ Settings</b>", show_kb("pshow_settings")); return
    ...
    if data.startswith("chrepv_"):
        parts = data.split("_"); row_s,rep = parts[1],parts[2]; row = int(row_s)
        if rep=="custom":
            ud["custom_days_for"]=("chrep",row); ud["custom_days_selected"]=[]
            r,msg,ds,ts,rs = row_detail(row)
            await safe_edit(q.message, f"{detail(msg,ds,ts)}\n\nSelect days:", custom_days_kb([],f"chrep_{row}")); return
        r,msg,ds,ts,rs = row_detail(row); sheet.update_cell(row,5,rep)
    ...
    if data.startswith("gchrepv_"):
        ...
        if rep=="custom":
            ud["custom_days_for"]=("gchrep",tid); ud["custom_days_selected"]=[]
            row,r = find_by_tid(tid)
            if not r: return
            msg,ds,ts,_ = get_detail(r)
            await safe_edit(q.message, f"{detail(msg,ds,ts)}\n\nSelect days:", custom_days_kb([],f"gchrep_{tid}")); return
        row,r = find_by_tid(tid)
        if not r: return
        sheet.update_cell(row,5,rep); msg,ds,ts,_ = get_detail(r)
        sub_info = gsub_text(tid)
    ...
    if data.startswith("cday_"):
        rest = data[5:]; last_us = rest.rfind("_")
        if last_us<0: return
        prefix, day_key = rest[:last_us], rest[last_us+1:]
        selected = ud.get("custom_days_selected",[])
        if day_key=="weekdays": selected = ["mon","tue","wed","thu","fri"]
        elif day_key=="all": selected = list(DAY_KEYS)
        elif day_key=="clear": selected = []
    ...
    if data.startswith("cdaysave_"):
        ...
        if not info: return
        kind,key = info
        ud.pop("custom_days_for",None); ud.pop("custom_days_selected",None)
        if kind=="chrep":
            row = int(key); r,msg,ds,ts,rs = row_detail(row); sheet.update_cell(row,5,rep_value)
            await safe_edit(q.message, f"{hdr('Updated ✓')}\n{detail(msg,ds,ts,fmt_rep(rep_value))}", home_kb())
            save_home(ud, q.message)
        elif kind=="gchrep":
            row,r = find_by_tid(key)
            if r:
                sheet.update_cell(row,5,rep_value); msg,ds,ts,_ = get_detail(r); sub_info = gsub_text(key)
    ...
    if data.startswith("cdayback_"):
        info = ud.get("custom_days_for"); ud.pop("custom_days_for",None); ud.pop("custom_days_selected",None)
        if info:
            kind,key = info
            if kind=="chrep":
                r,msg,ds,ts,rs = row_detail(int(key))
                await safe_edit(q.message, f"{hdr('Saved ✓')}\n{detail(msg,ds,ts)}\n\nRepeat?", rep_picker_kb(f"chrepv_{key}"))
            elif kind=="gchrep":
                row,r = find_by_tid(key)
                if r:
                    msg,ds,ts,_ = get_detail(r)
    ...
    if data.startswith("mw_"):
        parts = data[3:].split("_"); yr,mo,wi = int(parts[0]),int(parts[1]),int(parts[2])
        txt, kb = build_week_view(uid, yr, mo, wi, get_tz(uid))
        await safe_edit(q.message, txt, kb); return
    if data.startswith("mn_"):
        parts = data[3:].split("_"); yr,mo = int(parts[0]),int(parts[1])
        txt, kb = build_month_view(uid, yr, mo, get_tz(uid))
        await safe_edit(q.message, txt, kb); return
    ...
    if data.startswith("wrdet_"):
        parts = data[6:].split("_"); ws,we = parts[0],parts[1]
        try: rr = sheet.get_all_values()
        except: rr = []
        uid_s = str(uid)
        ...
        for v in rr[1:]:
            ...
            ds = norm_date(str(v[2]).strip()); st = str(v[5]).strip().lower()
            if ws<=ds<=we:
                msg_s = str(v[1]).strip()[:25]
                if st=="done": done_items.append(f"  ✅ {fmt_date(ds)} · {msg_s}")
                elif st=="missed": missed_items.append(f"  ✗ {fmt_date(ds)} · {msg_s}")
        lines = [f"{hdr('Weekly Detail')}\n{fmt_date(ws)} — {fmt_date(we)}\n"]
        if done_items: lines.append("<b>Completed</b>"); lines.extend(done_items[:15]); lines.append("")
        if missed_items: lines.append("<b>Missed</b>"); lines.extend(missed_items[:15]); lines.append("")
        if not done_items and not missed_items: lines.append("No data.")
        lines_text = "\n".join(lines)
        await safe_edit(q.message, lines_text, home_kb()); save_home(ud, q.message)
        return

    # Route to sub-handlers
    if data.startswith(("gjoin_","gskip_","gdone_","gsnzp_","gsnzb_","gsnz_")): await _btn_group(q,ctx,uid,data); return
...
async def _btn_group(q, ctx, uid, data):
    uid_s = str(uid); user = q.from_user; uname = get_username(user)
    if data.startswith("gjoin_"):
        tid = data[6:]; row,r = find_by_tid(tid)
        ...
        msg,ds,ts,rs = get_detail(r); rep = r[4] if len(r)>4 else "none"
        ...
        tid = data[6:]; st = next((s for u,_,s in get_tmembers(tid) if str(u)==uid_s),None)
        if st and st!="pending": await safe_edit(q.message,"<i>Already handled</i>"); return
        set_tstatus(tid,uid_s,"done"); await rm_gpm(ctx,tid,uid_s)
        row,r = find_by_tid(tid); msg = str(r[1]).strip() if r else ""
...
async def _btn_cal(q, ctx, ud, uid, data):
    utz = get_tz(uid); step = ud.get("step","")
    if data.startswith("cal_"):
        parts = data[4:].split("_"); yr,mo = int(parts[0]),int(parts[1])
        if step=="edit_date":
        ...
        if step=="edit_date":
            ...
            if is_past(ds,ts,utz):
                ...
                sent = await q.message.reply_text(f"{hdr('Updated ✓')}\n{msg}\nDate: {fmt_date(old_d)} → <b>{fmt_date(ds)}</b>\n{fmt_time(ts)} · {rs}",
                    reply_markup=home_kb(), parse_mode="HTML")
                save_home(ud, sent)
        elif step=="g_date":
            ud["date"] = ds; msg,ts = ud.get("message",""),ud.get("time")
            if ts:
                if is_past(ds,ts,utz):
                    now = datetime.now(utz)
                    await safe_edit(q.message, f"{hdr('Group Reminder')}\n{msg}\n{fmt_time(ts)}\n\n{past_msg(ts)}\nPick a future date:",
                ...
                sent = await ctx.bot.send_message(chat_id=q.message.chat.id,
...
async def _btn_edit(q, ud, uid, data):
    if data.startswith("emsg_"):
        ...
        await safe_edit(q.message, f"{hdr('Edit Reminder')}\n{msg}\nCurrent: <i>{fmt_date(ds)} · {fmt_time(ts)}</i>\n\nPick new date:",
            cal_kb(now.year,now.month,f"edit_{row}","« Back",tz=get_tz(uid)))
        ...
        await safe_edit(q.message, f"{hdr('Edit Reminder')}\n{detail(msg,ds,ts,rs)}\n\nWhat to change?",
            InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("Time", callback_data=f"etime_{row}"), InlineKeyboardButton("« Back", callback_data=f"view_{row}")]]))
...
async def _btn_cfg(q, ctx, ud, uid, data):
    if data=="cfg_digest_toggle":
        ...
        dt = fmt_time(cfg["digest_time"]) if not cfg["digest_on"] else "OFF"
        sent = await q.message.reply_text(f"Digest → <b>{'ON · '+fmt_time(cfg['digest_time']) if not cfg['digest_on'] else 'OFF'}</b>",
            reply_markup=home_kb(), parse_mode="HTML")
        save_home(ud, sent)
        ...
        await safe_edit(q.message, f"{hdr('Settings')}\nDigest time: <b>{fmt_time(cfg['digest_time'])}</b>\n\nEnter new time:\n<i>e.g. 7am, 8:30 AM</i>",
            InlineKeyboardMarkup([[InlineKeyboardButton("« Back", callback_data="cfg_close")]]))
        save_p(ud, q.message)
        ...
        val = int(data[5:]); save_cfg(uid,"max_retries",val)
        sent = await q.message.reply_text(f"Retries → <b>{val}×</b>", reply_markup=home_kb(), parse_mode="HTML")
        save_home(ud, sent)
        await show_settings(q.message, uid, ctx)
    elif data=="cfg_gap":
        cfg = get_cfg(uid)
        ...
        val = int(data[5:]); save_cfg(uid,"retry_gap",val)
        sent = await q.message.reply_text(f"Retry gap → <b>{val} min</b>", reply_markup=home_kb(), parse_mode="HTML")
        save_home(ud, sent)
        await show_settings(q.message, uid, ctx)
    elif data=="cfg_tz":
        cfg = get_cfg(uid); btns = []
        row = []
        ...
        if 0<=idx<len(TZ_DATA):
            ...
            sent = await q.message.reply_text(f"Timezone → <b>{tz_short(TZ_DATA[idx][0])}</b>", reply_markup=home_kb(), parse_mode="HTML")
            save_home(ud, sent)
            await show_settings(q.message, uid, ctx)
        ...
        if not grps:
            await safe_edit(q.message, f"{hdr('Groups')}\n\nNo subscriptions.",
                InlineKeyboardMarkup([[InlineKeyboardButton("- Close", callback_data="cfg_close")]])); return
        btns = []
        for gid in grps:
            try: chat = await ctx.bot.get_chat(int(gid)); name = chat.title or f"Group {gid}"
            except: name = f"Group {gid}"
        ...
        try:
            ...
            for i,r in enumerate(rows[1:],2):
                if str(r[0])==gid_s and str(r[1])==uid_s: grp_sheet.update_cell(i,5,"false"); break
        except: pass
        sent = await q.message.reply_text("Unsubscribed ✓", reply_markup=home_kb(), parse_mode="HTML")
        save_home(ud, sent)
        await show_settings(q.message, uid, ctx)

# ============= TEXT HANDLER ===============
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    step = ctx.user_data.get("step",""); text = update.message.text.strip()
    if update.effective_chat.type=="private": update_username(update.effective_user.id, get_username(update.effective_user))
    if update.effective_chat.type!="private":
        if step.startswith("g_") and str(update.effective_chat.id)==str(ctx.user_data.get("g_chat","")):
            # Check for tags in the group message step
            if step=="g_message":
                tags = extract_tags(update.message)
                if tags: ctx.user_data["g_tags"] = tags
                text = strip_mentions(text, update.message)
...
async def _do_step(update, ctx, step, text):
    ud, uid = ctx.user_data, update.effective_user.id; utz = get_tz(uid)
    if step=="message":
        ...
        if result and (result.get('time') or result.get('date')):
            ...
            ud["message"] = msg; ud["step"]="date"; now = datetime.now(utz)
            sent = await update.message.reply_text(f"{hdr('New Reminder')}\n{msg}\n\nPick a date:",
        ...
        if not row: return
        parsed = parse_time(text)
        if not parsed: await update.message.reply_text("Invalid time.\n<i>e.g. 9pm, 9:30 PM</i>",parse_mode="HTML"); return
        ...
        sent = await update.message.reply_text(f"Digest time → <b>{fmt_time(parsed)}</b>", reply_markup=home_kb(), parse_mode="HTML")
        save_home(ud, sent)
    elif step=="g_message":
        await del_prompt(ctx,ud)
        result = parse_nl_partial(text, tz=utz)
        if result and (result.get('time') or result.get('date')):
            ...
            msg = result['message'] if result else text
            ud["message"] = msg; ud["step"]="g_date"; now = datetime.now(utz)
...
async def send_and_track(ctx, chat_id, text, kb, track_key, track_cid):
    try:
        ...
        ctx.bot_data[track_key] = {"c":track_cid,"m":sent.message_id}; return True
    except Exception as e: logger.error(f"Send {chat_id}: {e}"); return False

async def snooze_cb(ctx: ContextTypes.DEFAULT_TYPE):
    row,chat = ctx.job.data["row"],ctx.job.data["chat"]
    try: r = sheet.row_values(row)
    except Exception as e: logger.error(f"snooze {row}: {e}"); return
...
async def auto_retry(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    cfg = get_cfg(uid_val); max_r,gap = cfg["max_retries"],cfg["retry_gap"]
    ...
    if count>=max_r:
        ...
        return
    await rm_btns(ctx,row); nc = count+1
...
async def grp_snooze_cb(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    set_tstatus(tid,uid_s,"pending"); row,r = find_by_tid(tid)
    if not r: return
    msg = str(r[1]).strip(); await rm_gpm(ctx,tid,uid_s)
...
async def grp_retry(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    cfg = get_cfg(creator); max_r,gap = cfg["max_retries"],cfg["retry_gap"]
    count = int(r[6]) if len(r)>6 and r[6].isdigit() else 0
...
async def check_weekly_report(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for r in cfg_rows[1:]:
        ...
        ws = (now-timedelta(days=7)).strftime("%Y-%m-%d"); we = now.strftime("%Y-%m-%d")
        uid_s = str(r[0]); done,missed = 0,0
        day_done, day_missed = {}, {}
        for v in rem_rows[1:]:
            if len(v)<6 or str(v[0])!=uid_s: continue
            if len(v)>7 and str(v[7]).strip(): continue
        ...
        best = max(day_done,key=day_done.get) if day_done else "—"
        worst = max(day_missed,key=day_missed.get) if day_missed else "—"
        mot = "Outstanding! 🏆" if pct>=90 else "Keep it up! 💪" if pct>=70 else "Room to improve 📈" if pct>=50 else "Let's do better 🎯"
        ws_d = (now-timedelta(days=7)).strftime("%-d %b"); we_d = now.strftime("%-d %b")
        txt = (f"📊 <b>Weekly Report</b>\n{DIV}\n{ws_d} — {we_d}\n\n"
               f"✅ Completed: {done}/{total} ({pct}%)\n❌ Missed: {missed}\n\n"
               f"📅 Most Productive: {best}\n📉 Most Missed: {worst}\n\n{mot}")
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("📋 Detail", callback_data=f"wrdet_{ws}_{we}"), InlineKeyboardButton("＋ New", callback_data="add")]])
        try: await ctx.bot.send_message(chat_id=uid_int,text=txt,reply_markup=kb,parse_mode="HTML")
...
async def check_reminders(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for idx,v in enumerate(vals[1:],2):
        ...
        uid_s = str(v[0]); user_tz = safe_tz(tz_map.get(uid_s,DEF_TZ)); now = datetime.now(user_tz)
        ...
        msg = str(v[1]).strip(); gid = str(v[7]).strip() if len(v)>7 else ""; tid = str(v[8]).strip() if len(v)>8 else ""
