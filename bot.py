...
# ============= CALENDAR ==================
def cal_kb(year, month, back_cb="cancel", back_txt="✕ Cancel", tz=None, cb_prefix="cal_", day_prefix="day_"):
    now = datetime.now(tz or safe_tz(DEF_TZ))
    ...
    for week in cal_module.monthcalendar(year, month):
        ...
        for day in week:
            if day == 0 or datetime(year, month, day).date() < now.date():
                ...
                row.append(InlineKeyboardButton(lbl, callback_data=f"{day_prefix}{ds}"))
        kb.append(row)
    td = now.strftime("%Y-%m-%d")
    tm = (now + timedelta(days=1)).strftime("%Y-%m-%d")
    kb.append([InlineKeyboardButton("Today", callback_data=f"{day_prefix}{td}"), InlineKeyboardButton("Tomorrow", callback_data=f"{day_prefix}{tm}")])
    ...
    if datetime(py, pm, 1) >= datetime(now.year, now.month, 1):
        nav.append(InlineKeyboardButton("‹", callback_data=f"{cb_prefix}{py}_{pm:02d}"))
    else:
        nav.append(InlineKeyboardButton(" ", callback_data="noop"))
    nav.append(InlineKeyboardButton("›", callback_data=f"{cb_prefix}{ny}_{nm:02d}"))
    kb.append(nav)
...
async def post_init(app):
    await app.bot.set_my_commands([
        ...
        BotCommand("reminders", "Schedule & reports"),
...
async def list_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        ...
        return
    # Private: redirect to unified reminders
    await reminders_cmd(update, ctx)

async def month_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_chat.type != "private":
        await update.message.reply_text("Use /reminders in private chat.")
        return
    # Private: redirect to unified reminders
    await reminders_cmd(update, ctx)
...
# ============= UNIFIED REMINDERS DASHBOARD =============

def compute_period_stats(uid, start_date, end_date):
    """Compute done/missed stats for any date range. Returns dict or None."""
    try:
    ...
    return {
        ...
        "day_done": day_done, "day_missed": day_missed,
...
def build_schedule_today(uid, utz):
    """Build the today + upcoming view for the schedule tab."""
    now = datetime.now(utz)
    
    reminders = get_user_reminders(uid)
    
    try:
        expanded = expand_recur(reminders, now.date(), now.date() + timedelta(days=7))
    except Exception:
        expanded = []
    ...
    lines = [f"<b>📅 Schedule</b>\n{DIV}\n"]
    
    # Today section
    lines.append(f"☀️ <b>Today</b> · {now.strftime('%-d %b, %a')}")
    today_items = by_date.get(now.date(), [])
    if today_items:
        for item in sorted(today_items, key=lambda x: x["time"]):
            ic = ST_IC.get(item["status"], "○")
            rep_s = f" · {fmt_rep(item['rep'])}" if item.get("rep") and item["rep"] != "none" else ""
            lines.append(f"  {ic} {fmt_time(item['time'])} · {item['msg'][:40]}{rep_s}")
    else:
        lines.append("  <i>No reminders today</i>")
    
    # Upcoming section
    lines.append(f"\n<b>📅 Upcoming</b>")
    shown = 0
    d = now.date() + timedelta(days=1)
    while d <= now.date() + timedelta(days=7) and shown < 7:
        day_items = by_date.get(d, [])
        if day_items:
            count = len(day_items)
            lines.append(f"  {d.strftime('%-d %b, %a')} · {count} reminder{'s' if count != 1 else ''}")
            shown += 1
        d += timedelta(days=1)
    
    if shown == 0:
        lines.append("  <i>No upcoming reminders</i>")
    
    total_active = sum(1 for r in reminders if str(r[5]).strip() in ("active", "pending", "snoozed"))
    lines.append(f"\n<b>{total_active} active</b>")
    
    return "\n".join(lines)

def build_schedule_day_view(uid, date_str, utz):
    """Build detailed view for a specific day with action buttons on each reminder."""
    try:
        target_date = datetime.strptime(date_str, "%Y-%m-%d").date()
    except Exception:
        return f"{hdr('Schedule')}\nInvalid date.", \
               InlineKeyboardMarkup([[InlineKeyboardButton("« Back", callback_data="sch_cal")]])
    
    reminders = get_user_reminders(uid)
    expanded = expand_recur(reminders, target_date, target_date)
    day_items = [item for item in expanded if item["date"] == target_date]
    
    lines = [f"<b>📅 {target_date.strftime('%-d %b, %a')}</b>\n{DIV}\n"]
    
    if not day_items:
        lines.append("<i>No reminders</i>")
        return "\n".join(lines), InlineKeyboardMarkup([
            [InlineKeyboardButton("« Calendar", callback_data="sch_cal"),
             InlineKeyboardButton("📋 All", callback_data="sch_list")]
        ])
    ...
    btns = []
    for idx, item in enumerate(sorted(day_items, key=lambda x: x["time"]), 1):
        ic = ST_IC.get(item["status"], "○")
        rep_s = f" · {fmt_rep(item['rep'])}" if item.get("rep") and item["rep"] != "none" else ""
        lines.append(f"<b>{idx}</b> {ic} {fmt_time(item['time'])} · {item['msg'][:35]}{rep_s}")
        
        # Find row for action buttons
        for ri, r in enumerate(all_rows[1:], 2):
            if (len(r) >= 6 and str(r[0]) == uid_s and
                str(r[1]).strip() == item["msg"] and
                norm_date(str(r[2]).strip()) == item["date"].strftime("%Y-%m-%d") and
                norm_time(str(r[3]).strip()) == item["time"]):
                st = str(r[5]).strip() if len(r) > 5 else "active"
                if st not in ("missed", "cancelled", "done"):
                    btns.append([
                        InlineKeyboardButton(f"{idx} ✓ Done", callback_data=f"done_{ri}"),
                        InlineKeyboardButton(f"{idx} ◷ Snooze", callback_data=f"snzp_{ri}")
                    ])
                break
    
    btns.append([
        InlineKeyboardButton("« Calendar", callback_data="sch_cal"),
        InlineKeyboardButton("📋 All List", callback_data="sch_list")
    ])
    
    return "\n".join(lines), InlineKeyboardMarkup(btns)


def build_reports_menu(uid, utz):
    """Build the reports period list."""
    now = datetime.now(utz)
    ...
    lines.append(f"► <b>This Week</b> ({ws.strftime('%-d %b')} – {we.strftime('%-d %b')})")
    
    # Last week
    lws = ws - timedelta(days=7)
    lwe = lws + timedelta(days=6)
    lines.append(f"► <b>Last Week</b> ({lws.strftime('%-d %b')} – {lwe.strftime('%-d %b')})")
    
    # This month
    lines.append(f"► <b>This Month</b> ({now.strftime('%B %Y')})")
    
    # Last month
    lm = now.replace(day=1) - timedelta(days=1)
    lines.append(f"► <b>Last Month</b> ({lm.strftime('%B %Y')})")
    
    # Discover past months from sheet data
    try:
        rem_rows = sheet.get_all_values()
    except Exception:
        rem_rows = []
    ...
    for v in rem_rows[1:]:
        ...
        try:
            ...
            mk = d.strftime("%Y-%m")
            if mk not in months_seen and d < now.replace(day=1):
                months_seen.add(mk)
        except Exception:
            pass
    
    past_months = sorted(months_seen, reverse=True)[:6]
    for mk in past_months:
        try:
            d = datetime.strptime(mk, "%Y-%m")
            lines.append(f"► {d.strftime('%B %Y')}")
        except Exception:
            pass
    
    if not past_months:
        lines.append("\n<i>No historical data yet</i>")
    
    # Buttons
    btns = [
        [InlineKeyboardButton("This Week", callback_data=f"rpt_week_{ws.strftime('%Y-%m-%d')}"),
         InlineKeyboardButton("Last Week", callback_data=f"rpt_week_{lws.strftime('%Y-%m-%d')}")],
        [InlineKeyboardButton("This Month", callback_data=f"rpt_month_{now.strftime('%Y-%m')}"),
         InlineKeyboardButton("Last Month", callback_data=f"rpt_month_{lm.strftime('%Y-%m')}")],
    ]
    
    month_btns = []
    for mk in past_months[:4]:
        try:
            d = datetime.strptime(mk, "%Y-%m")
            month_btns.append(InlineKeyboardButton(d.strftime("%b %Y"), callback_data=f"rpt_month_{mk}"))
        except Exception:
            pass
    if month_btns:
        btns.append(month_btns)
    
    btns.append([
        InlineKeyboardButton("« Schedule", callback_data="sch_today"),
        InlineKeyboardButton("🔴", callback_data="pclose_pshow_reminders")
    ])
    
    return "\n".join(lines), InlineKeyboardMarkup(btns)


def build_report_detail(uid, start_date, end_date, label, utz):
    """Build a summary report for any period."""
    stats = compute_period_stats(uid, start_date, end_date)
    if not stats or stats["total"] == 0:
        return (
            f"<b>📊 {label}</b>\n{DIV}\n\nNo reminders in this period.",
            InlineKeyboardMarkup([[
                InlineKeyboardButton("« Reports", callback_data="rpt_menu"),
                InlineKeyboardButton("📅 Schedule", callback_data="sch_today")
            ]])
        )
    ...
    else: mot = "Let's do better next time 🎯"
    
    txt = (
        f"<b>📊 {label}</b>\n{DIV}\n\n"
        f"✅ Completed: {stats['done']}/{stats['total']} ({pct}%)\n"
        f"❌ Missed: {stats['missed']}\n\n"
        f"📅 Most Productive: {stats['best_day']}\n"
        f"📉 Most Missed: {stats['worst_day']}\n\n"
        f"{mot}"
    )
    
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📋 Detail", callback_data=f"rpt_detail_{start_date.strftime('%Y-%m-%d')}_{end_date.strftime('%Y-%m-%d')}"),
         InlineKeyboardButton("« Reports", callback_data="rpt_menu")],
        [InlineKeyboardButton("📅 Schedule", callback_data="sch_today")],
    ])
    return txt, kb
...
def build_report_detail_view(uid, start_date, end_date, label, utz):
    """Build detailed report with individual reminder items."""
    try:
        rem_rows = sheet.get_all_values()
    except Exception:
        return f"<b>📊 {label}</b>\n{DIV}\n\nNo data.", \
               InlineKeyboardMarkup([[InlineKeyboardButton("« Reports", callback_data="rpt_menu")]])
    ...
    for v in rem_rows[1:]:
        ...
        if start_s <= ds <= end_s:
            ...
            if st == "done":
                done_list.append(f"  ✅ {fmt_date(ds)} · {fmt_time(ts)} · {msg[:35]}")
            elif st == "missed":
                missed_list.append(f"  ✗ {fmt_date(ds)} · {fmt_time(ts)} · {msg[:35]}")
    
    lines = [f"<b>📊 {label} — Detail</b>\n{DIV}\n"]
    if done_list:
...
async def reminders_cmd(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Unified reminders dashboard — Schedule + Reports tabs."""
    if update.effective_chat.type != "private":
        await update.message.reply_text("Use /reminders in private chat. Use /list for group reminders.")
        return
    ...
    txt = build_schedule_today(uid, utz)
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📅 Calendar", callback_data="sch_cal"),
         InlineKeyboardButton("📋 All List", callback_data="sch_list")],
        [InlineKeyboardButton("📊 Reports", callback_data="rpt_menu"),
    ...
    if update.message:
        ...
        sent = await update.callback_query.message.reply_text(txt, reply_markup=kb, parse_mode="HTML")
    
    show_cb = "pshow_reminders"
    ctx.bot_data[f"pmin_{sent.message_id}"] = {"min_text": "<b>📅 Schedule</b>", "show_cb": show_cb}
    ctx.job_queue.run_once(auto_minimize, AUTO_MIN_SEC, data={
        "c": sent.chat.id, "m": sent.message_id,
        "min_text": "<b>📅 Schedule</b>", "show_cb": show_cb
    })
...
async def show_list(target, uid, ctx, new=False):
    ...
    if not items:
        t = f"{hdr('Reminders')}\nNo reminders found."
        show_cb = "pshow_list"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("« Schedule", callback_data="sch_today"), InlineKeyboardButton("🔴", callback_data=f"pclose_{show_cb}")]])
        if new:
    ...
    if num_row:
        btns.append(num_row)
    show_cb = "pshow_list"
    nav_row = [InlineKeyboardButton("« Schedule", callback_data="sch_today")]
...
async def on_btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # Schedule tab: today/upcoming view
    if data == "sch_today":
        utz = get_tz(uid)
        txt = build_schedule_today(uid, utz)
        kb = InlineKeyboardMarkup([
    ...
    # Schedule tab: calendar sub-view (uses scal_ / sday_ prefixes)
    if data == "sch_cal":
        utz = get_tz(uid)
        now = datetime.now(utz)
        await safe_edit(q.message,
            f"<b>📅 Schedule Calendar</b>\n{DIV}\nPick a day:",
            cal_kb(now.year, now.month, "sch_today", "« Schedule", tz=utz,
                   cb_prefix="scal_", day_prefix="sday_"))
        return

    if data.startswith("scal_"):
        parts = data[5:].split("_")
        yr, mo = int(parts[0]), int(parts[1])
        utz = get_tz(uid)
        await safe_edit(q.message,
            f"<b>📅 Schedule Calendar</b>\n{DIV}\nPick a day:",
            cal_kb(yr, mo, "sch_today", "« Schedule", tz=utz,
                   cb_prefix="scal_", day_prefix="sday_"))
        return

    if data.startswith("sday_"):
        ds = data[5:]
        utz = get_tz(uid)
        txt, kb = build_schedule_day_view(uid, ds, utz)
        await safe_edit(q.message, txt, kb)
        return

    # Schedule tab: all list sub-view
    if data == "sch_list":
    ...
    if data == "rpt_menu":
        utz = get_tz(uid)
        txt, kb = build_reports_menu(uid, utz)
        await safe_edit(q.message, txt, kb)
        return

    if data.startswith("rpt_week_"):
        ds = data[9:]
        try:
        ...
        txt, kb = build_report_detail(uid, start, end, label, utz)
        await safe_edit(q.message, txt, kb)
        return

    if data.startswith("rpt_month_"):
        mk = data[10:]
        try:
            d = datetime.strptime(mk, "%Y-%m")
            start = d.date()
            end = d.replace(day=cal_module.monthrange(d.year, d.month)[1]).date()
        except Exception:
            return
        utz = get_tz(uid)
    ...
    if data.startswith("rpt_detail_"):
        rest = data[11:]
        parts = rest.split("_")
        if len(parts) >= 2:
            ...
            utz = get_tz(uid)
            label = f"{start.strftime('%-d %b')} – {end.strftime('%-d %b')}"
            txt, kb = build_report_detail_view(uid, start, end, label, utz)
            await safe_edit(q.message, txt, kb)
        return
    ...
    if data.startswith("pclose_"):
        ...
        if show_cb == "pshow_reminders":
            ...
            try:
                ...
                count = "?"
            await safe_edit(q.message, f"<b>📅 Schedule</b> ({count} active)", InlineKeyboardMarkup([[InlineKeyboardButton("📋 Show", callback_data=show_cb)]]))
        elif show_cb == "pshow_list":
            count = "?"
            try:
                uid_s = str(uid)
                rows = sheet.get_all_records()
    ...
    # Repeat change (private) - unchanged
...
async def _btn_rem(q, ctx, ud, uid, data):
    if data.startswith("view_"):
        ...
        btns.append([InlineKeyboardButton("« Back", callback_data="sch_list")])
...
async def check_weekly_report(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for r in cfg_rows[1:]:
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("📋 Detail", callback_data="wrdetail"), InlineKeyboardButton("📅 Reminders", callback_data="reminders")]])
        try:
...
def main():
    ...
    for cmd, fn in [("start", start), ("add", add_cmd), ("list", list_cmd), ("month", month_cmd), ("remind", remind_cmd), ("settings", settings_cmd), ("info", info_cmd), ("reminders", reminders_cmd)]:
